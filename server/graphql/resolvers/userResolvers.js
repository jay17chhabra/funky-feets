...
import { loginInputValidator, registerInputValidator } from '../../utils/vaildators.js';
...
export const users = {
  ...
  Mutation: {
    register: async (_, { registerInput: { username, email, password, confirmedPassword } }) => {
      const { valid, errors } = registerInputValidator(username, email, password, confirmedPassword);
      if (!valid) {
    ...
    updateUser: async (_, { updateUserInput: { email, username, firstName, lastName, shoeSize, password, currentPassword } }, context) => {
      ...
      if (user) {
        ...
        if (password) {
          if (password.length < 6 || currentPassword.length < 6) {
            throw new UserInputError('Password field must be minimum 6 letters');
          }
